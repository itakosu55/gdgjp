...
> **追記（2026-08-15）— 「AEC で消せない」は伝送の性質ではなく配線の性質である。**
>
> 当初この規則は無条件に `critical` としていた。理由は 9.1 の図のとおり「**別の join の
> スピーカー**を経由して戻ってきた音は参照信号を持たない」ことだが、これが成り立つのは
> 9.1 の形、すなわち**登壇者のノート PC が会場の中にいて、会場 PA から自分のマイクへ
> 戻る**場合である。自宅から内蔵マイク・内蔵スピーカーで参加している登壇者では、閉路の
> 帰り脚は `join.spk_out → 自分の内蔵スピーカー → 自宅の空気 → 自分の内蔵マイク →
> join.mic_in` であり、これは 9.7.4 が `warn` に下げると決めたまさにその形である。同じ
> lint 実行が同じホップを `remote-echo-acoustic` としては `warn` に落としながら、
> `transport-echo-loop` としては `critical` を出していた。
>
> よって伝送閉路も、**閉路上の音響ホップを 1 つずつ 9.7.4 の判定にかけ、1 つでも
> 覆われていれば `warn` へ下げる**。量化子が「いずれか」なのは、閉路はどこか 1 箇所の
> ゲインが落ちれば回らなくなるからで、13.4 の `reinforced`（部屋ごとの主張なので
> 「すべて」が要る）とは意味が違う。沈黙ではなく `warn` なのは、AEC は打ち消すので
> あって零にはしないからで、9.7.4 の「音量を上げると破綻する」と同じ意味論である。
>
> ホール側のホップが覆われないことは変わらないので、9.1 の形は `critical` のままである。
> また会場側の回り込みは引き続き `remote-echo-acoustic` が `critical` で報告するため、
> この降格で失われる情報はない。
...
> **追記（2026-08-15）— 判定は経路ではなくホップに属する。**
>
> 実装ではこの判定を「経路全体の形」ではなく「1 つの音響ホップの形」を問う述語
> (`selfCancelledHop`) として持つ。9.7.3 の追記のとおり伝送閉路の途中に現れるホップにも
> 同じことを訊く必要があり、そこでは経路は `join → 部屋 → join` の形をしていないため。
> ホップの両面の持ち主が同一で、そこから前後へ辿って最初に当たる会議アプリが同じ join で
> あり、その間に join・ホスト・両面の持ち主以外が立っていない、という条件は、すべて
> ホップの近傍だけで書ける。
...
> 塞いだ。`cable-bypasses-host` (warn) — 12.4.1 と同じ回、同じ語彙で片付けた (12.4.3)。
> 単独では書けなかった規則である: 正当な迂回（同一 PC 上の OBS が Meet の窓を掴む）と
> 結線ミスが同じ文書だった間は、どちらを指しても半分は誤報になる。`capture` という名前が
> 先に要り、それは割り当てを `links` から外す回にしか置けなかった。
...
> 12 章は実装済み。12.4.1（同一機械内の取り込み）は 11.8 の残る穴と同じ回で片付けた
> (12.4.3)。
...
#### 12.3.2. 実装時に決めたこと（実装済み）: 書けることと、そう書かれることは別

12.1 の欠陥は「1 本しか持てない」ことだった。テンプレートで持てるようにはなったが、**そう書かれる
保証はどこにもない**。結線フォームは既に埋まっている入力を候補から外さないし、キャンバスのドラッグ
も同じで、埋まっているジャックに 2 本目を落とせる。これは意図的である — 絵は配線面であり、成立
しない配線を止めるのはリンタの仕事である (10.5)。止めるのではなく、**指摘する**。

- **`shared-source-strip` (warn) を入れた。** 1 つのインスタンスに届く cable / capture / 割り当てを
  数え、2 本目以降があれば報告する。空間のホップは数えない — ソースは部屋を拾わない。
- **warn である理由。** その時点の書類は壊れていない。壊れるのは、誰かがその行の MONITOR を入れて
  会議の音を会場へ返した日で、そのとき会場マイクも一緒に返る。critical にすると「正しい配線でも
  消えない赤」になり、fix-one-and-re-run が回らなくなる (4.3)。安いうちに言うための警告である。
- **スコープはテンプレートのインスタンスだけ。** 物理ジャックには 12.3.1 の 2 つ目の理由がそのまま
  効く — 1 つのジャックに入った 2 系統は本当に同じフェーダーを共有する — し、そもそも「もう 1 つ
  作る」操作が存在しないので、提示できる修正がない。
- **`split-source` は、分割元の行のセルを複製する。モデルの既定値ではない。** PROGRAM から外して
  ある行が、行が変わっただけで配信へ戻るのでは、修正が事故を作ることになる。**どの行に座っているか
  だけを変えて他は何も変えない**、が無断で適用してよい条件である (4.3)。
- **対のテンプレートは対のまま増える** (`newSource` をそのまま使う)。片割れだけのブラウザソースは
  誰も欲しがらない (12.4)。
- **元の行に残るのは、割り当てがあればそれ、なければ最初のケーブル。** 割り当ては「そのアプリが
  その行で使っているデバイス」であって、後から乗ってきた取り込みより先にそこにいる。
- **`applyFix` はカタログを受け取るようにした。** 新しいソースを作るにはモデルが要る。`lint` と同じ
  形の lookup なので、生成 → lint → 修正 → lint のループは 1 つのカタログで回る。

採らない案: **ドラッグ時に暗黙にソースを作る。** 狙ったジャックに着くという約束を壊すし、「絵は
許し、リンタが判断する」という分担も壊す。わざと 1 行に 2 本入れて後で分ける、という順序も禁じて
しまう。
...
> 実装済み。図で 1 本へ畳む案だけ見送った (12.4.2)。

#### 12.4.2. 実装時に決めたこと（実装済み）

- **対は「向きごと」に組む。** 会議へ送る画面共有と会議から返ってくる画面共有は別の共有で
  あり、同じ `source_key = 'share'` の 4 ポートは 2 組になる。向きを無視して組むと、自分の
  送出と相手の返しを比べる意味のない指摘になる。
- **`cam` は「対」ではなく機能の名前である。** `cam_video_in` / `cam_video_out` に遡及付与は
  したが、向きごとに見ると音声側の相手がいない。カメラとマイクは別々に選ぶものであって
  1 つのソースではないからで、`sourceGroups` は要素 1 の組を落とす。カタログ表示には出る。
- **`partial-source` は「配信 PROGRAM への到達」で判定する。** 結線の有無ではない。行列で
  行を落としたソースは、何も刺さっていないソースと同じだけ配信に乗っていない。
- **同じ事故を 2 か所から捕まえる。** 会議側の組 (`share_audio_out` / `share_video_out`) は
  「取り込み忘れ」を、OBS 側の組 (`browser_audio:1` / `browser_video:1`) は「行列で片方だけ
  落とした」を指す。前者は会議ノードを、後者は OBS を名指しするので重複しない。
- **指摘文は媒体を明示する。** インスタンスの 2 つの半分は同じ名前を持つ (「Meet」が 2 つ)
  ので、ラベルだけではどちらを直せばよいか言えない。ラベルが媒体語を含まないときだけ補う。
- **図で 1 本へ畳む案は入れなかった。** 12.4 が「任意」と書いたとおりで、線が 1 本減ること
  の利得が測れていない。`LayoutEdge.sourceIds` の前例はあるので、必要になったときに入る。
...
> 実装済み。両方を同じ回で片付けた (12.4.3)。

#### 12.4.3. 実装時に決めたこと（実装済み）

括弧の中の案を採った。割り当てを `SetupNode.assignments: { port, hostPort }[]` へ移し、
`links` をケーブルだけの意味にしたうえで、同一機械内の取り込みに `capture` という第 3 の
エッジ種別を与えた。11.8 の残る穴はその副産物として塞がった。

- **向きは導出であり、二度と書かれない。** 機械の内側から見れば物理出力は *sink*、物理入力は
  *source* なので、向きの揃った 2 本のジャックはそれだけでどちらが上流かを言っている。
  `buildAssignmentEdges` がそれを読む。`links` にあった間は文書が向きを持つ必要があり、
  文書を書く者全員 — 今は人、のちに AI フェーズ — が「この関係だけは out→out で書く」を
  知っている必要があった。これを消すことが移動の本命であり、`isHostAssignment` と
  `orientHostAssignment` はともに不要になった。
- **`capture` は文書に何も足さない。** 「両端がソフトウェアで `hostNodeId` が一致する」は
  すでに書かれている事実であり、改めて宣言させれば間違える場所が 1 つ増えるだけである。
  リンクとして書かれ続けるのは、実際に out → in だからである。
- **`cable-bypasses-host` (warn) はホストを持つアプリにしか鳴らない。** ホスト未記入の join は
  対象外とする — 迂回すべき機械が存在せず、9.3 が「PC を誰も書かなかったミーティングは
  完成した文書である」と決めている。別々の機械上の 2 つのアプリを結ぶケーブルは、`capture`
  ではないので指摘に含まれる。
- **out→out のリンクは `bad-link-direction` になる。** 唯一の合法な例外が消えたので、これは
  「昔の書き方で書かれた割り当て」であり、そう言うほうが黙って受理するより有用である。
  文書のマイグレーションは書いていない: リモートへ配備したことがなく、存在する文書は
  fixture と seed とローカル dev だけである。
- **割り当ては `linkId` を持たない。** 図の `data-link-id` も付かず、テストはジャックで
  特定する。リンクではなくなったものにリンク id を与え続けるほうが後で高くつく。
...
#### 12.5.1. 実装時に決めたこと（実装済み）

代替案は採らなかった。12.3 の後に足した実コストは seed の migration 1 本と `coverage.ts` の
音源集合の定義だけである。

- **`origin` は「ポートごとに 1 音源」として数える。** マイクは何本ジャックがあっても 1 音源
  だが (ステレオペアは 1 本のマイクである)、1 つの OBS が動画を 2 つ持てば別々に「誰にも
  届いていない」がありうる。指摘が名指しすべきは行であって、アプリではない。
- **3 つの規則が同じ集合を見る。** `no-audio-to-stream` の音源がマイクだけだった間は、動画
  だけを流している OBS を「配信が無音」と誤って critical にしていた。12.6 が本題と呼んだ
  「動画を流したらリモートだけ無音」も、同じ集合になって初めて出る。
- **音源はまだ `mic` / `camera` カテゴリ ＋ `origin` に限る。** 11 章で結合がポートへ移った
  結果、USB スピーカーフォンの部屋側ジャックも音源だが、そこまで広げると会議アプリの
  `spk_out` (これも `from_space`) が音源に混ざり、「会議の音が会議に送られていない」という
  Mix-Minus そのものを指摘し始める。穴は 11 章由来であって `origin` が塞ぐものではない。
- **`paths.ts` は変更なし** (判断 C のまま)。`reachableFrom` は任意の頂点集合を始点に取る。
- **メディアしか持たない OBS に `software-io-unassigned` が鳴るのは正しい。** ホストのどの
  端子も使っていないのは事実で、その状態では会場の音を拾っていても他のどの規則にも見えない。
  `dangling-port` は端点カテゴリ限定なので、予告どおり黙る。
...
## 13. ハウリングは閉路では決まらない — 拡声の宣言

### 13.1. 現状の表現と、その代償

`acoustic-feedback-loop` は acoustic space を含む閉路を critical で報告する (4.2)。しかし
**閉路はハウリングの必要条件であって十分条件ではない**。成立条件はループゲインが 1 を超える
ことであり、それを決めるのはマイクとスピーカーの位置関係・指向性・ゲインストラクチャである。
どれもこのモデルは持っていない。3.5 が gain も EQ もフェーダーも持たないのは意図的な判断
なので、**十分条件は原理的に判定できない**。

代償は、いちばん普通の構成が critical になることである。

```
会場マイク → ミキサー MAIN → 会場スピーカー → 音響空間 → 会場マイク
```

これは拡声 (sound reinforcement) そのもので、PA のある会場では必ず閉じている経路である。
それを「ハウリングが発生する」と断定し、提示する修正は「MAIN から外す」「isolated にする」、
つまり**会場を無音にしろ**という助言になる。12.6 が coverage を critical にしなかった理由が
そのまま当てはまる — *正しい構成が消せない critical を出し続けると、「1 件直して再実行する」
運用が成立しない*。図の危険色を linter だけに持たせている理由 (10.5) も同じで、正しい構成を
赤くすると赤を無視する癖がつく。

現状に一貫した読みがないわけではない。12 章の合格条件 `hybridMonitorMix()` は会場マイクを
PA に返さず、PA には会議の音だけを出す構成で、そこでは `acoustic-feedback-loop` は出ない。
設計は暗黙に**「会場マイクは配信のためにあり、会場は生声で足りる」規模のイベント**を想定して
いる。妥当な想定ではあるが、それは会場規模の仮定であって物理ではなく、**どこにも書かれて
いない**。DevFest 規模のホール、ピンマイク、リモート登壇者の声を会場に出す場面では拡声は
必須で、閉路は意図的である。

### 13.2. 判断: 足りないのはレベルではなく意図である

不足している情報を「ゲイン」と読むとモデルにレベルを持ち込むことになり、3.5 と 4.1 の前提が
崩れる。しかも解決しない — ゲインベフォアフィードバックは卓の数値ではなく部屋と配置で決まる
ので、パッチシートに書ける情報ではない。

**足りないのは「この部屋は拡声する」という意図の宣言である。** これは人間が答えられる問いで
あり (「この会場は拡声しますか」に答えられない運営はいない)、機材のスペックでも数値でもなく、
当日の運用の記述である。9.3 が伝送空間について採ったのと同じ形 — *モデルが推測できないことは
既定で暗黙にし、必要なら明示させる*。

宣言があれば、`acoustic-feedback-loop` は「起きる」ではなく「起こり得る、そして起きない
ように運用されているはずだ」と言えるようになる。**検出は変えない。断定をやめる。**
...
export const spaceSchema = z.object({
  ...
  /**
   * この部屋は拡声する — 会場のマイクが会場のスピーカーから出る構成であり、
   * ハウリングは位置とゲインで抑えられている、という当日の運用の宣言。
   * `acoustic` でのみ意味を持つ。
   */
...
**なぜ空間か。** 閉路はマイクとスピーカーの*組*の性質だが、組は部屋の中で組み替わる。
スピーカーノードに持たせると、スピーカーが 2 本ある部屋で 2 回宣言することになり、
両者が食い違ったときに何が正しいか誰も言えない。ルーティング行に持たせるのは細かすぎて、
卓を触るたびに宣言が失効する。部屋は `venueKey` と同じ粒度の事実で、8 章でドキュメントが
トラックごとに分割されても生き残る。

**既定は未設定 = 拡声しない。** 既存ドキュメントは 1 文字も変わらず、挙動も今日と同一である。
`schemaVersion` は 1 のまま — 12.7 と同じく追加のみ。移行は不要。

**`acoustic` 以外に付いていても拒否しない。** `venueKey` が transport 空間に、`meetingKey` が
音響空間に付いていても schema が拒否していないのと同じ扱いにする。規則が `acoustic` でしか
読まないので害がなく、ここだけ refine を足すと検証の一貫性が崩れる。フォームは音響空間の
ときだけチェックボックスを出す。

### 13.4. 降格の条件

`acousticDiagnostic` は伝送 → 配信ソフト → 純音響の順に分岐する。**降格は最後の分岐だけに
効かせる。**

| 閉路 | ruleId | 宣言の効果 |
| --- | --- | --- |
| 会議を経由する | `transport-echo-loop` | **なし。critical のまま** |
| 配信ソフトのモニターを経由する | `stream-monitor-loop` | **なし。critical のまま** |
| 純粋な音響閉路 | `acoustic-feedback-loop` | 経路上の空間が**すべて** `reinforced` なら warn |
| 会議の音が部屋を回って戻る | `remote-echo-acoustic` | 同上 (2026-08-15 追記。13.7 を参照) |

前 2 者を除くのは、どちらも**位置とゲインでは直らない**からである。モニター出力が会場に
返っているのは誤設定であって拡声ではないし、伝送を跨ぐ回り込みは部屋の中の話ではない。
拡声の宣言はこの 2 つについて何も述べていない。

**この表は今も正しいが、`transport-echo-loop` が常に critical だという意味ではない。**
9.7.3 の追記のとおり、閉路上の音響ホップが join 自身の内蔵ペアであれば AEC の軸で warn へ
降りる。動いている軸が別なだけで、`reinforced` の効果は依然として「なし」である。ここが
交わらないことは意図的で、宣言が主張しているのは*ある部屋のループゲインが 1 を超えない
こと*、AEC が主張しているのは*ある 1 ホップの音が参照を持つこと*であり、別の事実である。

**「すべての空間が」でなければならない。** 宣言は部屋ごとなので、ホール A のマイクがホール B
のスピーカーへケーブルで回って戻る閉路は、A だけの宣言では覆えない。宣言していない部屋が
1 つでも経路にあれば critical のままにする。

```ts
const reinforced =
  spaceIds.length > 0 && spaceIds.every((id) => graph.spaces.get(id)?.reinforced === true);
```

音響閉路に visual 空間は乗らない (`SPACE_MEDIA.visual` は video だけで、探索は
`{ medium: "audio" }`) ので、種別で絞る必要はない。

**降格先は warn、`ruleId` は据え置く。** 9.7.4 の `remote-echo-acoustic` が
`isAecCancellable` で critical と warn を出し分けているのと同じ形にする。「通常は問題に
ならないが、音量を上げると破綻する」という意味論まで一致しており、そこで ruleId を分けて
いない以上ここでも分けない。`data-rule-id` は e2e の唯一の足がかり (CLAUDE.md) でもある。
4.2 の表は行を増やさず脚注を付ける。

文言は断定をやめ、当日の確認事項として書く。修正候補 (`routeFixes` / `isolationFixes`) は
降格後も**そのまま残す** — 拡声をやめるという判断は依然としてあり得る操作である。

### 13.5. 宣言は抑制ではない — 逆向きにも効く

これが lint 抑制コメントと違う点であり、この設計の要である。

`reinforced` は「この規則を黙らせる」ではなく「この部屋はこういう部屋だ」と言っている。
記述である以上、**逆向きの期待も生む**。12.6 が `source-not-reaching-room` を info に
留めた理由は「会場マイクの音は会場に出さない (それがハウリング対策である)」だった。
拡声すると宣言した部屋では、その理由が消える。

| 規則 | 既定 | `reinforced` な空間 |
| --- | --- | --- |
| `acoustic-feedback-loop` | critical | warn |
| `source-not-reaching-room` | info | **warn** |

拡声すると言った部屋でマイクが PA に乗っていないのは、ハウリング対策ではなく記入漏れか
結線漏れである。**宣言が findings を減らすだけでなく増やしもする**ので、赤を消すために
とりあえずチェックを入れる動機が生まれない。抑制フラグは構成が変わると黙って腐るが、
記述は腐ったときに別の findings として現れる。

### 13.6. 修正候補としての宣言と、AI 修復ループの境界
...
**`canApplyFix` は false を返す。** 4.3 の AI 修復ループ (生成 → Lint → 修正適用 → 再 Lint)
がこれを適用できてしまうと、**AI はあらゆるハウリング指摘をチェックボックス 1 つで消せる**。
これは値切ってはいけない一線で、`assign-space` と `add-device` が「人間が選ぶ必要があるので
適用せず提示に留める」側にいるのと同じ理由 — ただし理由は一段強く、

> **配線を変える修正は機械が適用してよい。世界についての事実を主張する修正は人間しか
> 適用できない。**

`canApplyFix` は既にこの境界線である。`declare-reinforced` はその 1 例目にすぎない。

いま `LintPanel` は `canApplyFix` を通らない Fix を**何も描画しない** (ノードへの「◯◯ を
開く」リンクだけがある)。宣言が発見不能では意味がないので、パネルに `hrefForSpace` を足し、
`declare-reinforced` を「(空間名) の設定を開く」リンクとして出す。ボタンではなくリンクなの
は正しい: 押して赤が消えるボタンより、部屋の設定で意図して入れるチェックのほうが、当日の
記録として良い。

### 13.7. `remote-echo-acoustic` は対象にしない

> **見出しは記録として残すが、結論は 2026-08-15 に反転した。** 節末の追記を先に読むこと。

12 章の合格条件は「消せない critical を 1 件残す」ことをテストで固定している —
会議の音が PA から出て、会場マイクが拾い、会議へ戻る経路である。13.1 と同じ論法でこれも
降格すべきに見えるが、**しない**。

`reinforced` が主張しているのは*ある部屋のループゲインが 1 を超えないこと*であって、
*エコーが聞こえないこと*ではない。ハウリングは発振してはじめて欠陥だが、エコーは発振の
はるか下のレベルで既に欠陥である。同じチェックボックスで両方を降格させるのは、異なる
2 つの物理条件を 1 つの宣言に畳むことになる。

会議室 DSP の AEC がこれを処理しているという宣言はあり得るが、それは別の事実
(`echoManaged` 相当) であり、置き場所も 9.7.4 の `isAecCancellable` である。ここでは
やらない。13 章のあとも、この 1 件は残る既知の穴である。

> 実装した。`DeviceModel.echoCancels` (migration 0010)。置き場所は予想どおり 9.7.4 で、
> 関数は `aecCanceller` になった。ただしそこへ触れて分かったのは、宣言以前に
> `speakerphoneMeeting()` が critical を出していたことである — 部屋の両面を 1 台が持つ形
> (11.3) を、判定が join とホストのポートに限っていたために取り落としていた。先にそれを
> 直し、そのうえで両面のどちらも持たない DSP を宣言で拾えるようにした。
>
> 降格を決めるのは宣言ではなく配線であり、部屋への往路と復路の両方に立っていることを要求
> する。だから 13.8 が却下した「カタログの boolean で severity を動かす」には当たらない。
> 宣言は findings を減らすだけでなく増やす: 片脚にしか立っていない機材には
> `aec-reference-missing` が出る (13.5)。`hybridMonitorMix()` の critical はそのままで、
> 12 章の合格条件は動いていない。
>
> **追記（2026-08-15）— 「両脚に立つ」から join 自身とそのホストを除く。** 経路は join の
> スピーカーで始まり join のマイクで終わるので、join は構造上つねに往路と復路の両方に立って
> いる。除外しないと、会議アプリのモデルに `echoCancels` を立てるだけであらゆる構成の
> `remote-echo-acoustic` が warn へ落ちる — 13.8 が却下した「カタログの boolean で severity
> を動かす」そのものである。join とホストは第 1 の形が「間に何も挟まらない」という厳しい
> 条件つきで既に裁いており、第 2 の形はもともと**どちらの面も持たない** DSP のための逃げ道
> なので、除外して意味が減る対象がない。
>
> あわせて降格時の文言を形ごとに分けた。「鳴らしている機材と拾っている機材が同じ」は第 1 の
> 形の事実であって、両面のどちらも持たない DSP では端的に偽である。
>
> **追記（2026-08-15）— この節の結論を反転し、`reinforced` の対象にする。** 上の物理の指摘
> (エコーは発振のはるか下で既に欠陥) は今も正しい。反転の理由は物理ではなく、規則の設計に
> ある。
>
> 1. **同じ 1 ホップを 2 つの重大度で報告していた。** 宣言済みの部屋で
>    `acoustic-feedback-loop` は「会場スピーカー → 空気 → 会場マイク」を warn と呼び、
>    `remote-echo-acoustic` は**同一のホップ**を含む経路を critical と呼んでいた。9.7.3 の
>    追記が `transport-echo-loop` について見つけた矛盾と同じ形で、軸が `reinforced` に
>    変わっただけである。
> 2. **critical が指す修正が、宣言そのものを否定していた。** この規則が出す
>    `isolationFixes` は「ヘッドセットにする」「会場スピーカーを isolated にする」の 2 つで、
>    拡声すると宣言した部屋ではどちらも部屋の目的を捨てる操作である。「correct wiring では
>    消せない critical は fix-one-and-re-run を壊す」— coverage 規則を warn 止まりにした
>    のと同じ判断が、ここにも当たっていた。
>
> **降格は「エコーが聞こえない」の主張ではない。** warn は沈黙ではなく、経路は今までどおり
> 名指しされ、修正候補も残る (13.4)。文言は降格時に別のものへ差し替え、消せない前提で
> 「マイクのゲインとスピーカーとの位置関係で抑えろ」と当日の確認事項として書く — 宣言済みの
> 部屋で「ヘッドセットにしろ」と言い続けるのは、助言として端的に無意味である。
>
> 量化子は 13.4 のまま**すべての空間**。加えて、経路に音響以外の空間が 1 つでも乗っていたら
> 降格しない (`declarableRooms` が null を返す) — 別の join のスピーカーを経由して戻る音に
> ついて、部屋の宣言は何も述べていないからである。13.4 の表の 2 行はこれで変わらない。
>
> **12 章の合格条件は動かない。** `hybridMonitorMix()` のホールは宣言していないので、
> critical を固定したテストはそのまま通る。反転しているのは「宣言した部屋では」という
> 条件付きの結論だけである。
>
> `echoManaged` 相当の別宣言 (上で示唆した案) は採らなかった。実際の会場はミキサーであって
> AEC を持つ会議室 DSP ではないので、正直に答えると宣言が偽になる。真実は「拡声していて、
> オンラインの音も PA に乗せる」であり、それは `reinforced` が既に言っていることである。
> チェックボックスをもう 1 つ増やして、当日誰も正しく答えられない質問をすることになる。

### 13.8. 採らない案

- **無条件に warn へ落とす。** いちばん安いが、意図しない誤パッチ (配信用のマイクを
  うっかり MAIN に上げた) が拾えなくなる。それは実際に起きる事故で、取り戻す手段がなくなる。
- **ゲインをモデルに持つ。** 3.5 の判断を覆すうえ、解決もしない (13.2)。
- **ミキサーのハウリングサプレッサをカタログの boolean にして自動降格する。**「鳴らない」も
  「鳴る」と同じくらい断定できない。サプレッサは鳴り始めてからノッチを入れる事後対策であり、
  搭載していない機種も多い。カタログのスペックで severity を動かすのは、道具にスペック表を
  信じさせることになる。
- **findings 単位の抑制リスト** (`ignore: [{ ruleId, nodeIds }]` をドキュメントに持つ)。
  構成が変われば黙って腐り、当日誰の役にも立つ情報を持たず、linter を黙らせる癖だけを残す。
  `reinforced` は編集を跨いで真であり続ける、部屋についての事実である。
- **スピーカーノードに持たせる / 会場規模から推測する。** 13.3 のとおり。

### 13.9. 変更の範囲

| 対象 | 変更 |
| --- | --- |
| `schema.ts` | `spaceSchema` に `reinforced`。`schemaVersion` は 1 のまま |
| `types.ts` | **変更なし** (`Space` は zod から導出) |
| `graph.ts` | **変更なし**。`graph.spaces` は既に `Space` をそのまま保持している |
| `rules/loops.ts` | `acousticDiagnostic` に降格の述語・severity・文言・`declare-reinforced` |
| `rules/coverage.ts` | `source-not-reaching-room` を `reinforced` な空間では warn へ (13.5) |
| `diagnostics.ts` | `Fix` に `declare-reinforced` |
| `mutations.ts` | `SetupOperation` に `update-space`、`applyFix` は `declare-reinforced` で doc を返す |
| `setup-intents.ts` | `update-space` インテント。`OPTIMISTIC` に追加 |
| `setup-inspector.tsx` | `SpaceInspector` にフォーム (音響空間のときだけチェックボックス) |
| `lint-panel.tsx` | `hrefForSpace` と `declare-reinforced` のリンク描画 |
| `fixtures.ts` / `lint.test.ts` | 降格する場合・部屋が片方だけ宣言されている場合・モニター閉路 |
| `e2e/seed-data.ts` | 新しい `e2e_setup_reinforced`。**`e2e_setup_howling` は宣言しない** |

**`update-space` が今日は存在しない。** 空間は追加と削除しかできず、`SpaceInspector` は
削除ボタン以外は読み取り専用である。`update-node` に倣って `patch` を取る形にし、
`id` と `kind` は除外する — 種別を変えるとそこに結合しているポートが宙に浮くので、
`update-node` が `deviceId` / `modelId` を除外しているのと同じ理由である。

**`clean()` の落とし穴には当たらない。** あれは `SetupNode` をホワイトリストから組み直す
関数で、空間には対応物がない。`update-space` はマージで実装し、`cleanSpace` は作らない。
作るなら CLAUDE.md の「新しいフィールドは `clean()` に足す」規則がそのまま空間にも及ぶ。

**`e2e_setup_howling` は変えない。** 正典のハウリングであり `linter.spec.ts` が critical を
アサートしている。`layout.ts` のランク付けが閉路を持つドキュメントを常用入力として要求して
いる (CLAUDE.md) 点も、トポロジは同一なのでどちらでも影響はない。

### 13.10. 代償

**記入項目が 1 つ増える。** 9.3 の「常用構成に余計な記入を課さない」に触れる。緩和は、
既定が今日の挙動と完全に同一であること、そして質問が**linter が訊く理由を持ったときにしか
現れない**こと (findings のリンクから空間の設定へ飛ぶ) である。

**宣言した部屋は、linter が見張るのをやめた部屋である。** これは正直な代償で、値切れない。
和らげているのは 3 点 — 降格先は warn であって沈黙ではない、経路は今までどおり名指しされる、
そして 13.5 のとおり宣言は別の findings を生む。

**宣言は主張であり、主張は間違い得る。** 4 月に宣言した構成を 11 月の別会場にコピーすれば
嘘になる。`venueKey` があるので将来は突き合わせられるが、いまは検出できない。

**8 章との相互作用。** 同じホールで 2 トラックが走るとき、両方のドキュメントが同じ宣言を
持っていないと、同じ部屋が一方では拡声で一方ではそうでないことになる。突き合わせの鍵は
`venueKey` で、クロストラック規則を書く回に「宣言の不一致」を findings にする。

### 13.11. 着手順

1. `spaceSchema` に `reinforced`、`rules/loops.ts` に降格の述語・severity・文言。
   `fixtures.ts` と `lint.test.ts` だけで完結し、**画面には何も出ない**。単独で検証できる
2. `update-space` (operation・インテント・`OPTIMISTIC`) と `SpaceInspector` のチェックボックス。
   ここで初めて人間が宣言できるようになる
3. `declare-reinforced` を Fix に足し、`LintPanel` に `hrefForSpace` を通す (13.6)
4. `source-not-reaching-room` を `reinforced` で warn へ (13.5)
5. e2e に `e2e_setup_reinforced` と `linter.spec.ts` のケース
6. CLAUDE.md のループ規則と「Known simplifications」を更新し、実装ノートに「実装時に
   決めたこと」を書く

1 と 2 の間で止めても壊れない (誰も宣言できないので今日と同じ挙動になる) ので、
そこが自然な区切りである。
