...
> 11.2–11.4 と 11.8 は実装済み (11.9-1,2,3)。ポート単位の `isolated` (11.6) と、内蔵マイク・
> スピーカーのモデル廃止 (11.9-4,5) は未着手。
...
残る穴も書いておく。`内蔵マイク.out → join.mic_in` のように**機械を迂回したケーブル**は
...
> 12.6 の 2 規則のみ実装済み (12.9-1)。表現の変更 — ポートテンプレート・`origin`・`sourceKey` —
> は未着手。
